...
object Homework:
  ...
  case class NonEmpty(elem: Int, left: IntSet, right: IntSet) extends IntSet:
    ...
    @targetName("union")
    infix def ∪(that: IntSet): IntSet = left ∪ right ∪ that.include(elem)

    @targetName("intersection")
     infix def ∩(that: IntSet): IntSet = 
      if (that contains elem) then NonEmpty(elem, left ∩ that, right ∩ that)
      else left ∩ that ∪ right ∩ that

    @targetName("complement")
    infix def ∖(that: IntSet): IntSet = 
      if (that contains elem) left ∖ that ∪ right ∖ that
